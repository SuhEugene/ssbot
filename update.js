...
const builds = [{ name: 'Paradise', value: 'para' }];

const commands = [
  {
    data: new ContextMenuCommandBuilder().setName('Погладить').setType(2),
    access: 'user'
  },
  {
    data: new ContextMenuCommandBuilder().setName('Плаг').setType(2),
    access: 'user'
  },
  {
  ...
  },
  {
    data: new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('запрос').setDescription('Теги e621 через пробел'))
      .addStringOption(o =>
        o
          .setName('категория')
  ...
  },
  {
    data: new SlashCommandBuilder()
      ...
      .addStringOption(o =>
        o.setName('название').setDescription('Текст названия файла').setRequired(true)
      )
      .addStringOption(o => o.setName('аватарка').setDescription('URL аватарки').setRequired(true)),
    access: 'user'
  },
  {
    data: new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('ckey').setDescription('ckey игрока').setRequired(true)),
    access: 'user'
  },
  {
    data: new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('сервер').setDescription('IP сервера').setRequired(true)),
    access: 'user'
  },
  {
    data: new SlashCommandBuilder()
      ...
      .addSubcommand(sub =>
        sub
          ...
          .addUserOption(o =>
            o.setName('игрок').setDescription('Игрок, которому выдаётся вайтлист').setRequired(true)
          )
          .addStringOption(o =>
            o
              .setName('раса')
      ...
      .addSubcommand(sub =>
        sub
          ...
          .addUserOption(o =>
            o.setName('игрок').setDescription('Игрок, кому выдать роль').setRequired(true)
          )
          .addStringOption(o =>
            o
              .setName('роль')
              ...
              .setRequired(true)
              .addChoices(
                ...ADMIN_ROLES.map(({ name, value }) => ({ name, value })).concat({
                  name: 'Снять',
                  value: 'demote'
                })
              )
      ...
      .addSubcommand(sub =>
        sub
          ...
          .addStringOption(o =>
            o.setName('t').setDescription('Токен для привязки').setRequired(true)
          )
      )
      .addSubcommand(sub => sub.setName('отвязать').setDescription('Отвязать аккаунт Baystation12'))
      .addSubcommand(sub =>
        sub
          .setName('игрок')
          .setDescription('Информация об игроке')
          .addUserOption(o =>
            o.setName('игрок').setDescription('Игрок, информацию о котором нужно узнать')
          )
      ),
    access: 'user'
  }
...
function getGlobCommands() {
  return commands
    .map(c => c.data.setDMPermission(true).toJSON())
    .filter(c => c.name.toLowerCase() === 'плаг')
    .map(c => {
...
function getCommands() {
  return commands
    .map(c => c.data.setDMPermission(c.access == 'user').toJSON())
    .map(c => {
...
client.on('ready', async () => {
  ...
  if (process.argv.includes('-get')) {
    console.log('========= GUILD =========');
    console.log(await rest.get(Routes.applicationGuildCommands(client.application.id, GUILD)));
    console.log('========= GLOBAL =========');
    console.log(await rest.get(Routes.applicationCommands(client.application.id)));
  }

  if (process.argv.includes('-c')) {
    console.log('Refreshing application (/) commands...');
    await rest.put(Routes.applicationGuildCommands(client.application.id, GUILD), {
      body: getCommands()
    });
    console.log(
      'ANSWER\n',
      await restv10.put(Routes.applicationCommands(client.application.id), {
    ...
    let r = await rest.get(Routes.applicationGuildCommands(client.application.id, GUILD));
    ...
    cmds = r.map(c => {
      ...
      return {
        id: c.id,
        name: c.name,
        role: cmd.access != 'user' ? config.roles[cmd.access] : false
      };
    });
  ...
  if (process.argv.includes('-i')) {
    ...
    await restv10.put(`/applications/${client.application.id}/role-connections/metadata`, {
      body: []
    });
    console.log('Successfully updated interactions\n');

    console.log('Started getting interactions...');
    let r = await restv10.get(`/applications/${client.application.id}/role-connections/metadata`);
