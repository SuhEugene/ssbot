const { escapeMarkdown, EmbedBuilder, ActionRowBuilder, ButtonBuilder, Client, GatewayIntentBits, Interaction } = require('discord.js');
...
app.post("/hookd", async (req, res) => {
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(commits.map(c => {
			...
			return `${url} ${emojTypes[type.toLowerCase()] || type} ${msg.join(":")}`+
			` - ${c.author.name}`
		}).join("\n"))
		.setColor("#A7A7A7")
	res.send();
	...
	sock.on("message", async msg => {
		...
		if (data.type == "dead") {
			...
			await ch.send({ embeds: [ 
				new EmbedBuilder().setTitle("Ded").setColor("DarkRed")
				.setDescription(
...
const doBayQuery = (query) => new Promise(
	(resolve, reject) => bayCon.query(
		query,
		(err, data, fields) => err ? reject({ err, data, fields }) : resolve({ err, data, fields })
	)
);
...
async function connectToBay () {
	...
	} catch (e) { console.error("Couldn't connect to bay station");console.error(e) }
...
client.on("messageCreate", async msg => {
	...
	// "Идёт" gif
	if (msg.content.includes("1169587906755694604"))
		return await msg.delete();
	// "Post this rat" gif
	if (msg.content.includes("993919553396543528"))
		return await msg.delete();
	...
	if (cmd == "!pats") {
		if (msg.author.id != "706124306660458507")
	...
	if (cmd == "!pat") {
		if (msg.author.id != "706124306660458507" && msg.author.id != "449288145574428673" && msg.author.id != "333588597234073601")
			return await msg.react("❌");
		try {
			pats = JSON.parse(fs.readFileSync("./pats.json")) || {};
		} catch (e) {
...
async function buildPara (interaction, embed) {
	...
	await interaction.editReply({ embeds: [ embed ] });
	execScript(`./buildpara.sh`).then(async r => {
...
async function pullPara (interaction, embed) {
	...
	await interaction.editReply({ embeds: [ embed ] });
	execScript(`./pullpara.sh`).then(async r => {
		embed.setColor("Blurple")
				 .setTitle("<:kokomi:990458395981144074> Успешный пулл")
		await interaction.editReply({ embeds: [ embed ] });
	}).catch(async e => {
		console.log(e);
...
async function resetPara (interaction, embed) {
	...
	await interaction.editReply({ embeds: [ embed ] });
	execScript(`./resetpara.sh`).then(async r => {
		embed.setColor("Blurple")
				 .setTitle("<:kokomi:990458395981144074> Успешный ресет")
		await interaction.editReply({ embeds: [ embed ] });
	}).catch(async e => {
		console.log(e);
		let output = `**Код ошибки:** ${e.error.code}`;
...
async function getPlayerDataByDiscord(discordId) {
	if (!(await connectToBay()))
		return ({ err: "Невозможно подключиться к базе данных BayStation" });

	const { err, data: users } = await doBayQuery(`SELECT * FROM erro_player WHERE discord_id="${discordId}"`);
...
async function ckeyByDiscord(discordId, interaction, embed) {
	...
	if (err) {
		interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		return false;
	} 
	if (!ckey) {
...
async function bayPlayerData(interaction, embed) {
	...
	return interaction.editReply({ embeds: [
		embed.setTitle("Информация об игроке").setColor("Blurple")
...
async function bayWhiteListToggle(interaction, embed) {
	...
	if (!ALL_RACES[race]) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Раса не существует")
	]});
	...
	const { err: err1, data: players } = await doBayQuery(`SELECT * FROM whitelist WHERE ckey="${ckey}" AND race="${race}"`);
	if (err1) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Ошибка запроса к базе данных")
	]});
	if (players.length) {
		const { err: err2 } = await doBayQuery(`DELETE FROM whitelist WHERE ckey="${ckey}" AND race="${race}"`);
		if (err2) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		await interaction.editReply({ embeds: [
			embed.setTitle(":x: Успешно удалено").setColor("DarkRed")
			.setDescription(`Игрок \`${ckey}\` успешно удалён из вайтлиста расы **${ALL_RACES[race] || "\`"+race+"\`"}**`)
		]});
	} else {
		const { err: err2 } = await doBayQuery(`INSERT INTO whitelist (ckey, race) VALUES ("${ckey}", "${race}")`);
		if (err2) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		await interaction.editReply({ embeds: [
			embed.setTitle(":white_check_mark: Успешно добавлено").setColor("DarkGreen")
			.setDescription(`Игрок \`${ckey}\` успешно добавлен в вайтлист расы **${ALL_RACES[race] || "\`"+race+"\`"}**`)
		]});
	}
...
async function bayAdminToggle(interaction, embed) {
	...
	if (!role && roleName != "demote") return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Роль не существует")
	]});
	...
	if (players.length) {
		...
		if (roleName == "demote") return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Игрок не найден как администратор - снять невозможно")
		]});

		const { err: err2 } = await doBayQuery(`INSERT INTO erro_admin (ckey, rank) VALUES ("${ckey}", "${role.bay}")`);
		if (err2) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		await interaction.editReply({ embeds: [
			embed.setTitle(`:white_check_mark: Успешное ${isUp ? 'повышение' : 'понижение'}`).setColor("DarkGreen")
			.setDescription(`Игрок \`${ckey}\` успешно ${isUp ? 'повышен' : 'понижен'} до роли **${role.bay}** и получает плашку <&${role.role}>`)
		]});
	}	
}
...
function checkToken(token) {
	let flag = true;
	for (let letter of token) if (!allowed.includes(letter)) {
		flag = false;
		break;
	}
	return flag;
...
async function bayLinkAccount (interaction, embed) {
	...
	if (token.length < 32 || !checkToken(token)) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Неверный токен")
	]});
	if (!(await connectToBay())) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Невозможно подключиться к базе данных BayStation")
	...
	// DISCORD CHECK
	const { err, data: discords } = await doBayQuery(`SELECT * FROM erro_player WHERE discord_id="${interaction.user.id}"`);
	if (err) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Ошибка запроса к базе данных")
	]});
	if (discords.length) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription(`Дискорд аккаунт <@!${interaction.user.id}> уже привязан к аккаунту \`${discords[0].ckey}\``)
	]});
	// /DISCORD CHECK

	const { err: err1, data: players } = await doBayQuery(`SELECT * FROM erro_player WHERE discord_id="${token}"`);
	if (err1) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Ошибка запроса к базе данных")
	]});
	...
	if (players.length) {
		const { err: err2 } = await doBayQuery(`UPDATE erro_player SET discord_id="${interaction.user.id}" WHERE ckey="${players[0].ckey}"`);
		if (err2) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		await interaction.editReply({ embeds: [
			embed.setTitle(":white_check_mark: Успешно привязано").setColor("DarkGreen")
			.setDescription(`Аккаунт \`${players[0].ckey}\` успешно привязан к дискорд аккаунту <@!${interaction.user.id}>`)
		]});
	} else {
		await interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
...
async function bayUnlinkAccount (interaction, embed) {
	...
	const { err, data: players } = await doBayQuery(`SELECT * FROM erro_player WHERE discord_id="${interaction.user.id}"`);
	if (err) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription("Ошибка запроса к базе данных")
	]});
	...
	if (players.length) {
		const { err: err2 } = await doBayQuery(`UPDATE erro_player SET discord_id="" WHERE ckey="${players[0].ckey}"`);
		if (err2) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Ошибка запроса к базе данных")
		]});
		await interaction.editReply({ embeds: [
			embed.setTitle(":x: Успешно отвязано").setColor("DarkRed")
			.setDescription(`Аккаунт \`${players[0].ckey}\` успешно отвязан от дискорд аккаунта <@!${interaction.user.id}>`)
		]});
	} else {
		await interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
...
async function getServerData(interaction, embed) {
	const [ ip, port ] = interaction.options.getString("сервер")
		.split("/").join("")
		.replace("byond:", "")
		.split(":");
	await interaction.editReply({
		content: `Данные сервера: \`byond://${ip}:${port}\`\n` +
...
async function checkServersStatus(interaction, embed) {
	...
	return await interaction.editReply({ embeds: [
                embed.setTitle("Статус серверов")
                .setColor("Blurple")
                .setDescription(formatServer("para", jsonData))
        ]});
}

const zeroes = ["Никого", "Хайпоп", "0", "9999", "Мяу", "Отсутствуют", "Нема", "Да", "Нет", "---", "UwU", "$#@", "284617", "-1", "Хуууууууй", "•~•", "._.", "0_о", "Существуют?", "Не существуют", "Миф", "Играют в Майнкрафт", "Исчезли", "Успешно аннигилированы", "Забанены", "Уже вышли", "Листают NSFW каналы", "Милашки", "Продались", "Спят", "<:KannaKms:873199424032092200>", "<:NinoPain:1094445733408682034>", "<:pepesadge:1145111506795642900>", "<a:Bye:1042544645986586634>", "-0", "o", "ö", "∅"];
async function getServerPlayers(interaction, embed) {
	...
	const jsonData = JSON.parse(r);
	return await interaction.editReply({ embeds: [
		embed.setTitle(`Онлайн игроки: ${jsonData.length || randZero}`)
		.setColor("Blurple")
...
async function checkServers (interaction, embed) {
	let servers = {};
	try { servers.para = await byondFetch.fetchTopic({ ip: "127.0.0.1", port: 49229, topic: "?ping" }); }
	catch (e) { servers.para = null; }
	try { servers.bay = await byondFetch.fetchTopic({ ip: "127.0.0.1", port: 49383, topic: "?ping" }); }
	catch (e) { servers.bay = null; }
	try { servers.skyrat = await byondFetch.fetchTopic({ ip: "127.0.0.1", port: 49191, topic: "?ping" }); }
	catch (e) { servers.skyrat = null; }
	console.log(servers);
	
	return await interaction.editReply({ embeds: [ new EmbedBuilder().setTitle("Статус серверов")
		.setColor("Blurple")
		.setDescription(
...
async function getServerIP(interaction, embed) {
	...
	if (server) {
		if (!serverPorts[server]) return interaction.editReply({ embeds: [
			embed.setTitle("Ошибка").setColor("Red")
			.setDescription("Сервер не найден")
		]});
		if (server == "paran")
			return interaction.editReply({ embeds: [
				embed.setTitle('IP сервера')
				.setColor("Blurple")
				.setDescription(
...
async function giveMeetRole(interaction, embed) {
	...
	if (!roles.meet[role]) return interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
	...
	if ((await interaction.member.fetch()).roles.cache.has(roles.meet[role])) {
		...
		return interaction.editReply({ embeds: [
			embed.setTitle(":x: Роль успешно снята")
		...
		return interaction.editReply({ embeds: [
			embed.setTitle(":white_check_mark: Роль успешно добавлена")
...
function getFurryUrl(post) {
  return `https://static1.e621.net/data/${post.md5.slice(0, 2)}/${post.md5.slice(2, 4)}/${post.md5}.${post.file_ext}`
}
...
function getFurryPost(interaction, embed, post, count, query=[], rating=null, update=false) {
	const row = new ActionRowBuilder()
		.addComponents(
			new ButtonBuilder()
			.setCustomId('furry-new')
			.setLabel('Заменить')
			.setEmoji('♻️')
			.setStyle(1),
			new ButtonBuilder()
			.setCustomId('furry-place')
			.setLabel('Оставить')
			.setEmoji('👌')
			.setStyle(3),
			new ButtonBuilder()
			.setCustomId('furry-remove')
			.setLabel('Убрать')
			.setEmoji('✖️')
			.setStyle(4),
		);
	...
	return ({
		embeds: [
			embed.setTitle(`Фурря #${post.id}${count > 1 ? ' — '+count : ''}`)
			.setDescription(`**Теги:** ${tags.join(', ')}\n**Рейтинг:** ${post.score}\n**Категория:** ${fullRating[post.rating]}`)
...
async function sendNewFurryPost(interaction, embed, query, rating, count=1) {
	...
	if (!post)
		return interaction.editReply({ embeds: [
                        embed.setTitle("Фурря не найдена")
                        .setDescription(
	...
	try {
		const buttonClick = await reply.awaitMessageComponent({ filter: i => i.user.id === interaction.user.id, time: 120_000 });
		if (buttonClick.customId === 'furry-place')
		...
		console.error(e);
		try { await reply.edit({ components: [] }) } catch (e) { console.error(e); }
	}
}
...
async function createCustomPlug(interaction, embed) {
	...
	console.log("Creating plug", name);
	const plugPath = await plugger(name, url);
	console.log("Plug created", name);

	if (plugPath) {
		fs.copyFileSync(plugPath, path.join('/var/www/html/plugs/', path.basename(plugPath)));
		fs.unlinkSync(plugPath);
		return interaction.editReply({ embeds: [
			embed.setTitle("Гиф готова")
			.setDescription(`Скачать её можно по ссылке:\nhttps://download.wetskrell.ru/plugs/${path.basename(plugPath)}`)
			.setColor("Blurple")
		]});
	} else {
		return interaction.editReply({ embeds: [
			embed.setTitle("Всё хуйня")
...
async function startServer(interaction, embed) {
	...
	if (sentData.error) return await interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription(sentData.msg)
	]});
	await interaction.editReply({ embeds: [
		embed.setTitle("<a:andance:983035371900260422> Команда запуска отправлена").setColor("Greyple")
	]});
	const receivedData = await waitForSockResponse(server);
	console.log(receivedData);
	if (receivedData.error) return await interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
		.setDescription(receivedData.msg)
	]});
	return await interaction.editReply({ embeds: [
		embed.setTitle(":+1: Сервер запускается").setColor("Blurple")
	]});
}
...
async function stopServer(interaction, embed) {
	...
	await interaction.editReply({ embeds: [
		embed.setTitle("<a:andance:983035371900260422> Команда остановки отправлена").setColor("Greyple")
	]});
	const receivedData = await waitForSockResponse(server);
	if (receivedData.error) return await interaction.editReply({ embeds: [
		embed.setTitle("Ошибка").setColor("Red")
	...
	return await interaction.editReply({ embeds: [
		embed.setTitle(":+1: Сервер стопнут").setColor("DarkRed")
	]});
}
...
async function runIfHasRoles(roles, fun, interaction, embed) {
	...
	if (!hasRole) return await interaction.editReply({ content: "Нет доступа <:achE:974586173307641857>" });
...
client.on("interactionCreate", async (interaction) => {
	...
	if (interaction.commandName.toLowerCase() == "погладить") {
		...
		if (!pats[usrId]) return interaction.reply({
	...
	if (interaction.commandName.toLowerCase() == "плаг") {
		try {
			const mbr = interaction.guild && await interaction.guild.members.fetch(interaction.targetMember.id);
			const usr = interaction.targetUser;
			if (!mbr?.avatar && !usr.avatar) return interaction.reply({
			...
			if (!plugPath) return interaction.editReply({
...
client.on("interactionCreate", async (interaction) => {
	...
	if (interaction.commandName == "билд") return await runIfHasRoles([ roles.coder ], buildPara, interaction, embed);
	if (interaction.commandName == "пулл") return await runIfHasRoles([ roles.coder ], pullPara, interaction, embed);
	if (interaction.commandName == "ресет") return await runIfHasRoles([ roles.coder ], resetPara, interaction, embed);
	if (interaction.commandName == "бэй") {
		const sub = interaction.options.getSubcommand();
		if (sub == "вайтлист")
			return await runIfHasRoles([ roles.admin, "1069693040475771000" ], bayWhiteListToggle, interaction, embed);
		if (sub == "роль")
			return await runIfHasRoles([ roles.admin, roles.hadmin ], bayAdminToggle, interaction, embed);
		if (sub == "привязать")
			return await bayLinkAccount(interaction, embed);
		if (sub == "отвязать")
			return await bayUnlinkAccount(interaction, embed);
		if (sub == "игрок")
			return await bayPlayerData(interaction, embed);
	}

	if (interaction.commandName == "старт") return await runIfHasRoles([ roles.coder, roles.admin, roles.hadmin ], startServer, interaction, embed);
	if (interaction.commandName == "стоп") return await runIfHasRoles([ roles.coder, roles.hadmin ], stopServer, interaction, embed);
	if (interaction.commandName == "sstat") return await runIfHasRoles([ "1167210351604019231" ], getServerData, interaction, embed);
	if (interaction.commandName == "где") return await runIfHasRoles([ roles.coder, "1169672685266149496" ], require("./wherePlayer.js"), interaction, embed);
	if (interaction.commandName == "плаг") return await runIfHasRoles([ "1031627532660908114" ], createCustomPlug, interaction, embed);
